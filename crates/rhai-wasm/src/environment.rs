    ...
    fn from(val: JsValue) -> Self {
        Self {
            ...
            js_cwd: js_sys::Reflect::get(&val, &JsValue::from_str("js_cwd"))
                .unwrap()
                .into(),
            js_discover_rhai_config: js_sys::Reflect::get(
                &val,
                &JsValue::from_str("js_discover_rhai_config"),
            )
            .unwrap()
            .into(),
            js_is_dir: js_sys::Reflect::get(&val, &JsValue::from_str("js_is_dir"))
                .unwrap()
                .into(),
            js_sleep: js_sys::Reflect::get(&val, &JsValue::from_str("js_sleep"))
                .unwrap()
                .into(),
            js_url_to_file_path: js_sys::Reflect::get(
                &val,
                &JsValue::from_str("js_url_to_file_path"),
            )
            .unwrap()
            .into(),
        }
...
impl Environment for WasmEnvironment {
    ...
    fn discover_rhai_config(&self, from: &Path) -> Option<std::path::PathBuf> {
        let path_str = JsValue::from_str(&from.to_string_lossy());
        let this = JsValue::null();
        let res: JsValue = self
            .js_discover_rhai_config
            .call1(&this, &path_str)
            .unwrap();
    ...
    fn url_to_file_path(&self, url: &Url) -> Option<std::path::PathBuf> {
        let path_str = JsValue::from_str(url.as_str());
        let this = JsValue::null();
        let res: JsValue = self.js_url_to_file_path.call1(&this, &path_str).unwrap();
    ...
    fn is_dir(&self, root: &Path) -> bool {
        let path_str = JsValue::from_str(&root.to_string_lossy());
        let this = JsValue::null();
        let res: JsValue = self.js_is_dir.call1(&this, &path_str).unwrap();
        res.is_truthy()
    }
    ...
    async fn sleep(&self, duration: std::time::Duration) {
        ...
        let res: JsValue = self
            .js_sleep
            .call1(&this, &JsValue::from_f64(duration.as_millis() as _))
            .unwrap();

        let _ = JsFuture::from(Promise::from(res))
            .await
            .map_err(|err| anyhow!("{:?}", err));
    }
}
